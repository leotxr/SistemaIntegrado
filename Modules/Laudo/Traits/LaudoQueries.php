...
trait LaudoQueries
{
    ...
    public function queryReports($medicos_selecionados, $setores_selecionados)
    {
        ...
        return $this->queryBase()
            ...
            ->select(DB::raw("FATURA.DATA AS DATA_EXAME, FATURA.HORA AS HORA_EXAME, FATURA.ENTREGADATA AS DATA_ENTREGA, FATURA.PACIENTEID AS PACIENTEID, SUBSTRING(PACIENTE.NOME, 1, 5) AS PACIENTENOME, PROCEDIMENTOS.DESCRICAO AS EXAME, MEDICOS.NOME AS MEDICO, SETORES.DESCRICAO AS SETOR, FATVOICE.ARQUIVO AS DITADO, FATURA.LAUDOREAOK AS DIGITADO, FATURA.LAUDOASSOK AS ASSINADO"));
    }
    ...
    public function queryWithoutDoctor($setores_selecionados)
    {
        ...
        return $this->queryBase()
            ...
            ->where('FATURA.UNIDADEID', 1)
            ->whereIn('SETORES.DESCRICAO', $setores_selecionados)
            ->select(DB::raw("FATURA.DATA AS DATA_EXAME, FATURA.HORA AS HORA_EXAME, FATURA.ENTREGADATA AS DATA_ENTREGA, FATURA.PACIENTEID AS PACIENTEID, SUBSTRING(PACIENTE.NOME, 1, 5) AS PACIENTENOME, PROCEDIMENTOS.DESCRICAO AS EXAME, SETORES.DESCRICAO AS SETOR, FATVOICE.ARQUIVO AS DITADO, FATURA.LAUDOREAOK AS DIGITADO, FATURA.LAUDOASSOK AS ASSINADO"));
    }
