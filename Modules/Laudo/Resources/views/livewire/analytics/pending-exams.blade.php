    ...
    <div class="grid justify-end text-end p-2">
        <div>
            <x-secondary-button
                ...
                <x-icon name="filter" class="w-5 h-5 text-gray-500 dark:text-gray-100"></x-icon>
                filtros
            </x-secondary-button>
        </div>
    </div>
    <div>
        @livewire('laudo::analytics.exams-without-report', ['start_date' => $start_date, 'end_date' => $end_date])
    </div>
    <div class="mt-4">
        @livewire('laudo::analytics.exams-without-signature', ['start_date' => $start_date, 'end_date' => $end_date])
    </div>
    <div class="mt-4 mb-16">
        @livewire('laudo::analytics.exams-to-review', ['start_date' => $start_date, 'end_date' => $end_date])
    </div>
</div>
